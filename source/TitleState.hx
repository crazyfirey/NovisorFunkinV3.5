...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(1000, FlxG.height * 0.52).loadGraphic(Paths.image('titlelogo'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['Novisor Series', 'by'], -40);
				case 7:
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Remastered Update'); // credTextShit.text += '\nFunkin';
    ...
    function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			new FlxTimer().start(0.01, function(tmr:FlxTimer)
				{
					FlxTween.tween(logoBl, {'scale.x': 1, 'scale.y': 1}, 1, {ease: FlxEase.quartInOut});
			...
			new FlxTimer().start(0.01, function(tmr:FlxTimer)
				{
					if(logoBl.angle == -360) 
						FlxTween.angle(logoBl, logoBl.angle, 0, 4, {ease: FlxEase.quartInOut});
				}, 0);
